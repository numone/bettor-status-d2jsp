...
function showMedList(){
	...
	$('BODY DIV.tbb DIV.links B').append('<a id="BSMedListLink" href="javascript:void(0);">Poker Med List</a>');
...
function showMediators(sportObj,link){
	var offset = $(link).offset();
	$('#bsFieldset').css({top:offset.top,left:offset.left}).show();
	$('#bsFieldset LEGEND SPAN').html('Poker Med List');
	$('#bsFieldset DIV.main').html('');
...
function showFlushLink(){
	...
	$('BODY DIV.crt.links').append(' | Poker Status version: ' + VERSION.toFixed(2) + ', <span id="BSBottomSpan">cached for ' + timeAgo + ' minutes <a href="javascript:void(0);" id="BSBottomFlush">flush</a></span>');
	...
	GM_xmlhttpRequest({
		...
		onload:function(response){
			...
			if(LIST.version != VERSION){
				showErrorMsg('Your poker status script is out of date');//, <a href="http://userscripts.org/scripts/show/50720" target="_blank">Click Here</a> to upgrade!');
			}
			gatherSport(0);
		},
		onerror:function(err){
